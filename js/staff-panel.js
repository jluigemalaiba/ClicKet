(() => {
  ...
  function openPersonDetails(id) {
    ...
    modalBody.innerHTML = `<div class="staff-person-detail"><header><span class="staff-person-avatar">${escapeHtml((person.name || 'U').split(/\s+/).map(part => part[0]).join('').slice(0, 2))}</span><div><h3>${escapeHtml(person.name || 'Unnamed user')}</h3><p>${escapeHtml(person.email || '')}</p></div><span class="staff-status ${archived ? 'is-muted' : 'is-success'}">${archived ? 'Archived' : 'Active'}</span></header><section><h4>Account information</h4><div class="staff-person-detail__grid"><div><span>Username</span><strong>${escapeHtml(person.name || '')}</strong></div><div><span>Role</span><strong>${escapeHtml(role)}</strong></div><div><span>Account ID</span><strong>${escapeHtml(person.id || '')}</strong></div><div><span>Created</span><strong>${escapeHtml(person.created_at || '')}</strong></div></div></section><section><h4>Access & assignment</h4><div class="staff-person-detail__grid"><div><span>Venue scope</span><strong>${escapeHtml(venueScope)}</strong></div><div><span>Account status</span><strong>${archived ? 'Archived / disabled' : 'Active'}</strong></div></div></section></div>`;
    modal.hidden = false;
  }
