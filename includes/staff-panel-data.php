...
function clicketStaffPanelPayload(array $staff): array {
    ...
    $paidStatuses = ['paid', 'payment verified'];
    $pendingStatuses = ['pending', 'pending payment', 'for verification', 'payment submitted'];
    $paidOrders = array_values(array_filter($orders, static fn (array $order): bool => in_array(strtolower((string) ($order['payment_status'] ?? '')), $paidStatuses, true)));
    $pendingOrders = array_values(array_filter($orders, static fn (array $order): bool => in_array(strtolower((string) ($order['payment_status'] ?? '')), $pendingStatuses, true)));
